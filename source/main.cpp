...
void Imgui_SetDefaultTheme(void) {
...
int Init_Imgui(void) {
	...
	IM_ASSERT(font != NULL);
	Imgui_SetDefaultTheme();
	Textures_Load();
	return 0;
}

void Term_Imgui(void) {
	Textures_Free();
...
int main(int argc, char *argv[]) {
	...
	if (R_FAILED(ret = Init_Services()))
		return ret;
	
	if (R_FAILED(ret = Init_Imgui()))
		return ret;

	if (R_FAILED(ret = GUI_RenderMainMenu(window)))
		return ret;
